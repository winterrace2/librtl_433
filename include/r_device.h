...
#ifndef extdata_t
    typedef struct _extdata extdata_t;
#endif
...
enum modulation_types {
    ...
    OOK_DEMOD_MAX_VAL =             10, ///< Dummy. OOK demodulation must end at this value.
...
typedef struct r_device {
    ...
    float sync_width;
    float tolerance;
    int (*decode_fn)(struct r_device *decoder, struct bitbuffer *bitbuffer, extdata_t *ext);
