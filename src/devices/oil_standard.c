...
*/
static int oil_standard_decode(r_device *decoder, bitbuffer_t *bitbuffer, unsigned row, unsigned bitpos, extdata_t *ext)
{
    data_t *data;
    uint8_t *b;
    uint16_t unit_id;
...
static int oil_standard_callback(r_device *decoder, bitbuffer_t *bitbuffer, extdata_t *ext) {
    ...
    while ((bitpos = bitbuffer_search(bitbuffer, 0, bitpos, (uint8_t *)&preamble_pattern0, 16)) + 78 <=
        ...
        events += oil_standard_decode(decoder, bitbuffer, 0, bitpos + 14, ext);
        bitpos += 2;
    }
