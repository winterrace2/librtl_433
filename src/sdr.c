...
static int rtltcp_open(sdr_dev_t **out_dev, int *sample_size, char *dev_query, int verbose)
{
    ...
    if (ret != 12) {
        rtl433_fprintf(stderr, "Bad rtl_tcp header (%d)\n", ret);
        return -1;
    }
    if (strncmp(info.magic, "RTL0", 4)) {
        info.tuner_number = 0; // terminate magic
        rtl433_fprintf(stderr, "Bad rtl_tcp header magic \"%s\"\n", info.magic);
        return -1;
    }
    ...
    rtl433_fprintf(stderr, "rtl_tcp connected to %s:%s (Tuner: %s)\n", host, port, tuner_name);
...
int sdr_set_antenna(sdr_dev_t *dev, char *antenna_str, int verbose)
{
    ...
    if (dev->soapy_dev) {
        ...
        if (verbose) {
            if (r < 0)
                rtl433_fprintf(stderr, "WARNING: Failed to set antenna.\n");

            // report the antenna that is actually used
            rtl433_fprintf(stderr, "Antenna set to '%s'.\n",
  ...
  // currently only SoapySDR supports devices with multiple antennas
    rtl433_fprintf(stderr, "WARNING: Antenna selection only available for SoapySDR devices\n");
...
int sdr_apply_settings(sdr_dev_t *dev, char const *sdr_settings, int verbose)
{
...
#ifdef SOAPYSDR
    if (!dev->soapy_dev) {
        rtl433_fprintf(stderr, "WARNING: sdr settings only available for SoapySDR devices.\n");
        return -1;
    }
    ...
    for (size_t i = 0; i < settings.size; ++i) {
        ...
        if (verbose)
            rtl433_fprintf(stderr, "Setting %s to %s\n", key, value);
        if (!strcmp(key, "antenna")) {
            if (SoapySDRDevice_setAntenna(dev->soapy_dev, SOAPY_SDR_RX, 0, value) != 0) {
                r = -1;
                rtl433_fprintf(stderr, "WARNING: Antenna setting failed: %s\n", SoapySDRDevice_lastError());
            }
        }
            ...
            if (SoapySDRDevice_setBandwidth(dev->soapy_dev, SOAPY_SDR_RX, 0, (double)f_value) != 0) {
                r = -1;
                rtl433_fprintf(stderr, "WARNING: Bandwidth setting failed: %s\n", SoapySDRDevice_lastError());
            }
        }
        else {
            if (SoapySDRDevice_writeSetting(dev->soapy_dev, key, value) != 0) {
                r = -1;
                rtl433_fprintf(stderr, "WARNING: sdr setting failed: %s\n", SoapySDRDevice_lastError());
            }
        }
    }
